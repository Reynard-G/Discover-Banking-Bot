...
module.exports = {
  ...
  run: async (client, interaction) => {
    try {
      ...
      // Query transaction
      const creditcard_id = await creditcards.id(client, userID);
      await client.query(`INSERT INTO transactions (user_id, amount, cr_dr, status, note, creditcard_id, created_user_id, updated_user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [userID, amount, "DR", 1, note, creditcard_id, userID, userID]);
      await client.query(`INSERT INTO transactions (user_id, amount, cr_dr, status, note, created_user_id, updated_user_id) VALUES (?, ?, ?, ?, ?, ?, ?)`, [receivingAccountID, amount, "CR", 1, note, userID, userID]);
      ...
      await receivingUser.send({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`You have received a payment of **$${amount}** from <@${interaction.user.id}> (${interaction.user.displayName})`)
            .addFields(
      ...
      const receivingDisplayName = receivingUser.username;
      return interaction.editReply({
        embeds: [
          new EmbedBuilder()
            .setTitle("Credit Card Payment Sent")
            .setDescription(`You have sent a payment of **$${amount}** to <@${receivingDiscordID}> (${receivingDisplayName}).`)
            .addFields(
              { name: "Note", value: note ?? "No note provided" }
            )
            .setColor("Green")
